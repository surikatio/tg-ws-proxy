...
| `--lan` | выкл. | Раздать прокси на всю локальную сеть: слушать `0.0.0.0` и хранить постоянный секрет. [Подробнее](./LanServer.md) |
| `--secret` | `random` | 32-значный hex-ключ для авторизации клиентов |
| `--secret-file` | | Хранить секрет в файле и переиспользовать между запусками (включён при `--lan`) |
| `--max-connections` | `2048` | Предел одновременных подключений клиентов, `0` — без предела |
| `--transparent` | выкл. | Прозрачный режим: обслуживать клиентов без настройки прокси (только Linux, нужны маршруты и netfilter). [Подробнее](./Transparent.md) |
